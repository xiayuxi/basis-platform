			...
			elements : function() {
				...
				return $([]).add(this.currentForm.elements).filter(":input")
						...
						.not(this.settings.ignore).filter(function() {
							...
							// return false; ========zhuchenglin 2011.9.15
...
function showErrors() {
	...
	for (var i = 0; this.errorList[i]; i++) {
		...
		if (errdiv.length == 0) {
			errdiv = $('<div>' + '<div class="errmsgdiv fl errmsg"></div>'
					+ '</div>');
			errdiv.attr({
...
jQuery.extend(jQuery.validator.messages, {
			...
			number : "请填写有效的数字!",
			digits : "请填写有效整数!",
			creditcard : "请填写有效的信用卡号!",
			equalTo : "请再次填写相同的值!",
			accept : "请填写合法后缀名的字符串!",
			maxlength : jQuery.validator.format("请填写长度最多是 {0} 的字符串!"),
			minlength : jQuery.validator.format("请填写长度最少是 {0} 的字符串!"),
			rangelength : jQuery.validator.format("请填写长度介于 {0} 和 {1} 之间的字符串!"),
			range : jQuery.validator.format("请填写介于 {0} 和 {1} 之间的值!"),
			max : jQuery.validator.format("请填写最大为 {0} 的值!"),
			min : jQuery.validator.format("请填写最小为 {0} 的值!")
		});

/*
 * added by jianghy , 2011-9-9
 * 采用不严格的方式验证电话号码。只要电话号码由数字或者数字加"-"(最多两个"-")组合构成，均符合。比如：400-823-823， 95555,
 * 010-81567415, 18857107619
 */
// jQuery.validator增加不严格的电话号码验证
jQuery.validator.addMethod("lenientTel", function(value, element) {
			return this.optional(element)
					|| (/^([0-9]{1,9}(\-)?)?([0-9]{1,9}){1}(\-[0-9]{1,9})?$/
							.test(value));
		}, "电话号码格式错误!");

/*
 * added by jianghy , 2011-9-15 验证密码格式： 由字母、数字或者下划线组成，长度是6到12位 @param pwd
 * @return
 */
jQuery.validator.addMethod("pwd", function(value, element) {
			return this.optional(element)
					|| (/^[a-zA-Z0-9_]{6,12}$/.test(value));
		}, "密码格式错误!");
